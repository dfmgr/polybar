...
# @@Created          :  Monday, May 01, 2023 11:38 EDT
# @@File             :  build.sh
# @@Description      :
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
APPNAME="polybar"                    # Set build name
BUILD_NAME="${BUILD_NAME:-$APPNAME}" # Set build name
VERSION="202304292252-git"           # Set version
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
POLYBAR_LOG_DIR="${POLYBAR_LOG_DIR:-$HOME/.local/log/builds}"                           #
BUILD_SCRIPT_SRC_DIR="${BUILD_SCRIPT_SRC_DIR:-$HOME/.local/share/${BUILD_NAME}/source}" # set the source dir
BUILD_LOG_FILE="${BUILD_LOG_FILE:-$POLYBAR_LOG_DIR/${BUILD_NAME}_build.log}"            # set log files
BUILD_LOG_DIR="$(dirname "$BUILD_LOG_FILE")"                                            # get the log directory
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set bash options
trap 'exitCode=${exitCode:-0};[ -n "$POLYBAR_TEMP_FILE" ] && [ -f "$POLYBAR_TEMP_FILE" ] && rm -Rf "$POLYBAR_TEMP_FILE" |&__devnull;exit ${exitCode:-0}' EXIT
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
__logr() {
  eval "$*" 2>"$POLYBAR_LOG_DIR/$APPNAME.log.err" >"$POLYBAR_LOG_DIR/$APPNAME.log"
  return $?
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__notifications() {
  ...
  [ "$POLYBAR_NOTIFY_ENABLED" = "yes" ] || return
  [ "$SEND_NOTIFICATION" = "no" ] && return
  (
    set +x
    export SCRIPT_OPTS="" _DEBUG=""
    export NOTIFY_GOOD_MESSAGE="${NOTIFY_GOOD_MESSAGE:-$POLYBAR_GOOD_MESSAGE}"
    export NOTIFY_ERROR_MESSAGE="${NOTIFY_ERROR_MESSAGE:-$POLYBAR_ERROR_MESSAGE}"
    export NOTIFY_CLIENT_ICON="${NOTIFY_CLIENT_ICON:-$POLYBAR_NOTIFY_CLIENT_ICON}"
    export NOTIFY_CLIENT_NAME="${NOTIFY_CLIENT_NAME:-$POLYBAR_NOTIFY_CLIENT_NAME}"
    export NOTIFY_CLIENT_URGENCY="${NOTIFY_CLIENT_URGENCY:-$POLYBAR_NOTIFY_CLIENT_URGENCY}"
    notifications "$@"
    retval=$?
    return $retval
  ) |& __devnull &
